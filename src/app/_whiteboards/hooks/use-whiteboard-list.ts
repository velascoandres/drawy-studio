...
export const useWhiteboardList = ({ query }: Options = { query: { spaceId: undefined } }) => {  
  ...
  const openUpdateWhiteboard = (whiteboard: Omit<Whiteboard, 'content'>) => {
    openModal({
      component: CreateUpdateWhiteboard,
      props: {
        whiteboard
  ...
  const openRemoveWhiteboard = (whiteboard: Omit<Whiteboard, 'content'>) => {
    openModal({
  ...
  const openDetachSpace = (whiteboard: Omit<Whiteboard, 'content'>) => {
    openModal({
